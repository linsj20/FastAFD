...
public:
    ...
    void compile(const std::string &code, const std::filesystem::path& dir_path,
        ...
        const bool host_debug = get_env<int>("DG_JIT_HOST_DEBUG", 0);
        if (host_debug)
            std::cerr << "[deepgemm_jit] put begin " << code_path << std::endl;
        put(code_path, code);
        if (host_debug)
            std::cerr << "[deepgemm_jit] put done " << code_path << std::endl;
        ...
        if (host_debug)
            std::cerr << "[deepgemm_jit] command ready " << command << std::endl;
        if (get_env("DG_JIT_DEBUG", 0) or get_env("DG_JIT_PRINT_COMPILER_COMMAND", 0))
            printf("Running NVCC command: %s\n", command.c_str());
        if (host_debug)
            std::cerr << "[deepgemm_jit] command begin" << std::endl;
        const auto [return_code, output] = call_external_command(command);
        if (host_debug)
            std::cerr << "[deepgemm_jit] command done return_code=" << return_code
                      << " output_bytes=" << output.size() << std::endl;
        if (return_code != 0) {
            printf("NVCC compilation failed (return_code=%d)\n"
                   "  command: %s\n"
                   "  code: %s\n"
                   "  cubin: %s\n"
                   "  output:\n%s\n",
                   return_code, command.c_str(), code_path.c_str(),
                   cubin_path.c_str(), output.c_str());
            fflush(stdout);
