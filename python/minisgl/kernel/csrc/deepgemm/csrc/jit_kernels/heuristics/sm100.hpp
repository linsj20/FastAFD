...
struct SM100ArchSpec {
    ...
    static std::vector<Layout> get_layout_candidates(const GemmDesc& desc) {
        ...
        // Experimental sweep override: keep only candidates with the given
        // block N (and force swap_ab off) to probe memory-bound skinny-m
        // shapes where the wave/multicast preference picks too few CTAs.
        if (const int force_bn = get_env<int>("DG_FORCE_BLOCK_N"); force_bn > 0) {
            std::vector<Layout> filtered;
            for (const auto& c : candidates)
                if (c.block_n == force_bn and not c.swap_ab)
    ...
    static LayoutInfo get_layout_info(const GemmDesc& desc, const Layout& layout) {
        ...
        // SMs held by a co-resident kernel (e.g. the MegaMoE AG comm kernel
        // pinned on MINISGL_MEGAMOE_AG_SMS): wave counts must be computed on
        // the SMs actually available, else a "1-wave" pick (e.g. 144 CTAs on
        // a 148-SM device) silently becomes 2 waves next to a 24-SM spinner.
        const auto reserved_sms = get_env<int>("MINISGL_MEGAMOE_AG_SMS", 0);
        const auto effective_sms = std::max(desc.num_sms - reserved_sms, 1);
        const auto num_waves = ceil_div(num_blocks, effective_sms);
        const auto num_last_blocks = num_blocks % effective_sms;
        const auto last_wave_util = num_last_blocks == 0 ? effective_sms : num_last_blocks;
        // num_cycles doubles as a "memory-bound skinny m" flag: such GEMMs
        // are weight-stream-bound, so CTA count (bandwidth) beats multicast.
        // Measured m=46: n=9216 best at block_n=64 (144 CTAs, 6.6us) and
        // n=4096 at block_n=32 (128 CTAs, 9.1us) vs 8.2/9.9us default.
        const int64_t skinny_m = desc.get_expected_m() <= 64 ? 1 : 0;
        return {num_waves, last_wave_util, skinny_m, layout};
    }
    ...
    static bool compare(const LayoutInfo& a, const LayoutInfo& b) {
        ...
        // Doing multicast is better (but irrelevant for memory-bound skinny
        // m, where maximizing resident CTAs i.e. last_wave_util wins)
        if (not (a.num_cycles and b.num_cycles))
